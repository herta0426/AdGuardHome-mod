...
- **SNI 阻断**（`sni_filter` 配置段，默认关闭，仅 Linux）。它读取每条 TLS 连接开头的 ClientHello（明文），取出 SNI，命中过滤规则就注入 TCP RST，让连接在十几毫秒内失败；用的是**同一套过滤规则**，不需要另外维护清单。把包送进 NFQUEUE 的 netfilter 规则由模块脚本维护（`scripts/iptables.sh` 的 `AGH_SNI` 链），程序自己只开队列、读包、判决、发 RST，所以内核需要支持 `connbytes`、`NFQUEUE`。每条连接的 SNI 都会记进**查询日志**（放行的也在），被拦的显示为「已阻止（SNI 拦截）」。规则细节见 [doc/AdGuardHome.yaml.example](doc/AdGuardHome.yaml.example) 的 `sni_filter` 段与 [交接文档](HANDOVER.md) 第 2.6 节。
