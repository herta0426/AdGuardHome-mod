...
- **SNI 阻断**（`sni_filter` 配置段，默认关闭，仅 Linux）。在 `filter` 表的 `OUTPUT` 链上用 NFQUEUE 读取每条 TLS 连接开头的 ClientHello（明文），取出 SNI，命中过滤规则就注入 TCP RST，让连接在十几毫秒内失败。用的是**同一套过滤规则**，不需要另外维护清单。每条连接的 SNI 都会记进**查询日志**（放行的也在），被拦的显示为「已阻止（SNI 拦截）」。需要 root（Magisk 模块本来就以 root 运行）与内核支持 `connbytes`、`NFQUEUE`；缺任何一个只会记日志，不影响 DNS 服务。配置项与原理见 [doc/AdGuardHome.yaml.example](doc/AdGuardHome.yaml.example) 的 `sni_filter` 段与 [交接文档](HANDOVER.md) 第 2.6 节。
