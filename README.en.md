...
- **SNI blocking** (the `sni_filter` section, disabled by default, Linux only).  It reads the ClientHello at the beginning of every TLS connection, extracts the plain-text SNI, and, if the filtering rules match, injects a TCP RST that makes the connection fail in a dozen milliseconds.  It reuses **the same filtering rules**, so no separate list is needed.  The netfilter rules that send the packets to the NFQUEUE are installed by the module script (the `AGH_SNI` chain in `scripts/iptables.sh`), while the program only opens the queue, reads the packets, decides, and resets; the kernel therefore needs `connbytes` and `NFQUEUE` support.  The SNI of every connection is recorded in the **query log**, including the allowed ones; the blocked ones are shown as "Blocked (SNI)".  See the `sni_filter` section of [doc/AdGuardHome.yaml.example](doc/AdGuardHome.yaml.example) and section 2.6 of [HANDOVER.md](HANDOVER.md).
